...
const currentSeasonDB = db.getCurrentSeasonConnection();
const mainDB = db.getMainConnection();
const hash = require('object-hash');

api.get('/:kname?', function (req, res) {
  var kname = req.params.kname;
  currentSeasonDB.query("SELECT * FROM kingdoms WHERE name=?", [kname], function (err, result) {
    if (err) throw err;
...
api.post('/createKingdom', function (req, res) {
  ...
  mainDB.query("SELECT * FROM userAccounts WHERE username=?", [owner], function (error, result) {
  ...
  currentSeasonDB.query("SELECT * FROM kingdoms WHERE owner=?", [username], function (err, result) {
    if (err) throw err;
...
api.post("/deleteKingdom/:kname?/:pswd?/", function(req, res) {
  ...
  currentSeasonDB.query("SELECT * FROM kingdoms WHERE name=?", [kname], function(error, result) {
    if (error) throw error;
    let kingdom = result[0];
    if (kingdom == null || kingdom == undefined) responseManager.kingdomNotExist(res);
  })

  mainDB.query("SELECT * FROM userAccounts WHERE password=?", [pswd], function(error, result) {
    if (error) throw error;
    let user = result[0];
    if (user == null || user == undefined) responseManager.accountNotExist(res);
  })

  currentSeasonDB.query("SELECT * FROM kingdoms WHERE name=?", [kname], function(error, kingdom) {
    if (error) throw error;
    let username = kingdom[0].owner;
    mainDB.query("SELECT * FROM userAccounts WHERE username=?", [username], function(error, user) {
